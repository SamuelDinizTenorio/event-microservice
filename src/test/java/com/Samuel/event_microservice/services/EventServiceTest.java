...
class EventServiceTest {
    ...
    private EventRequestDTO createEventDTO(String title, LocalDateTime date, int maxParticipants) {
    ...
    void createEvent_withValidData_shouldReturnCreatedEvent() {
        // Arrange
        EventRequestDTO eventDTO = createEventDTO("Evento Válido", LocalDateTime.now().plusDays(10), 100);
        ...
        assertEquals(eventDTO.title(), createdEvent.getTitle());
    ...
    void getAllEvents_shouldReturnPageOfEvents() {
        ...
        Event event = new Event(createEventDTO("Evento Qualquer", LocalDateTime.now(), 100));
    ...
    void getUpcomingEvents_shouldCallRepositoryAndMapResult() {
        // Arrange
        Pageable pageable = PageRequest.of(0, 10);
        Event upcomingEvent = new Event(createEventDTO("Evento Futuro", LocalDateTime.now().plusDays(5), 100));
    ...
    void getEventDetails_withValidId_shouldReturnEventDetails() {
        ...
        Event event = new Event(createEventDTO("Evento Detalhado", LocalDateTime.now().plusDays(1), 100));
    ...
    void registerParticipant_withValidData_shouldSucceed() {
        // Arrange
        UUID eventId = UUID.randomUUID();
        Event event = new Event(createEventDTO("Evento para Registro", LocalDateTime.now().plusDays(1), 10));
        ...
        verify(emailServiceClient, times(1)).sendEmail(any());
    ...
    void registerParticipant_whenAlreadyRegistered_shouldThrowSubscriptionAlreadyExistsException() {
        // Arrange
        UUID eventId = UUID.randomUUID();
        Event event = new Event(createEventDTO("Evento com Duplicado", LocalDateTime.now().plusDays(1), 10));
        SubscriptionRequestDTO subscriptionDTO = new SubscriptionRequestDTO("test@example.com");
    ...
    void registerParticipant_whenEventIsFull_shouldThrowEventFullException() {
        // Arrange
        UUID eventId = UUID.randomUUID();
        EventRequestDTO dto = createEventDTO("Evento Lotado", LocalDateTime.now().plusDays(1), 1);
        Event fullEvent = new Event(dto);
    ...
    void registerParticipant_whenEmailFails_shouldStillRegister() {
        // Arrange
        UUID eventId = UUID.randomUUID();
        Event event = new Event(createEventDTO("Evento com Falha de Email", LocalDateTime.now().plusDays(1), 10));
        SubscriptionRequestDTO subscriptionDTO = new SubscriptionRequestDTO("test@example.com");

        when(eventRepository.findById(eventId)).thenReturn(Optional.of(event));
        when(subscriptionRepository.findByEventAndParticipantEmail(event, "test@example.com")).thenReturn(Optional.empty());
        // Simula uma falha no cliente de e-mail
        doThrow(new RuntimeException("Email service is down")).when(emailServiceClient).sendEmail(any());
    ...
    void getRegisteredParticipants_withValidEventId_shouldReturnParticipantsPage() {
        // Arrange
        UUID eventId = UUID.randomUUID();
        Event event = new Event(createEventDTO("Evento com Participantes", LocalDateTime.now().plusDays(1), 100));
